...
describe('SlackChannel', () => {
  ...
  describe('message handling', () => {
    ...
    it('routes threaded replies to a thread-scoped conversation JID', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);
      await channel.connect();
    ...
    it('starts a thread-scoped conversation when message ends with >>', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);
      await channel.connect();

      const event = createMessageEvent({
        ts: '1704067210.000000',
        text: '@Jonesy vllm 에 관한 문의 >>',
      });
      await triggerMessageEvent(event);

      expect(opts.onMessage).toHaveBeenCalledWith(
        'slack:C0123456789::thread:1704067210.000000',
        expect.objectContaining({
          chat_jid: 'slack:C0123456789::thread:1704067210.000000',
          content: '@Jonesy vllm 에 관한 문의',
        }),
      );
    });
    ...
    it('starts a thread-scoped conversation when message ends with &gt;&gt;', async () => {
      ...
      const event = createMessageEvent({
        ts: '1704067211.000000',
        text: '@Jonesy vLLM 에 관한 추가 질문 &gt;&gt;',
      });
      await triggerMessageEvent(event);

      expect(opts.onMessage).toHaveBeenCalledWith(
        'slack:C0123456789::thread:1704067211.000000',
        expect.objectContaining({
          chat_jid: 'slack:C0123456789::thread:1704067211.000000',
          content: '@Jonesy vLLM 에 관한 추가 질문',
        }),
      );
    });
  });
  ...
  describe('sendMessage', () => {
    ...
    it('sends thread replies using thread_ts', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);
      await channel.connect();
